jb.mapTest = {
    ...
    fadeSprite: function(sprite) {
        jb.assert(sprite, "Can't fade a 'null' sprite!");

        var col = this.colFromX(sprite.bounds.l);
        if (col <= 0) {
            sprite.spriteSetAlpha(Math.sqrt(Math.max(0, (sprite.bounds.l - this.origin.x) / (this.tileSize * this.scale))));
        }
        else if (col >= this.map.length - 2) {
            sprite.spriteSetAlpha(Math.sqrt(Math.max(0, (this.canvas.width - (sprite.bounds.l - this.origin.x + sprite.bounds.w)) / (this.tileSize * this.scale))));
        }
        else {
            sprite.spriteSetAlpha(1.0);
        }
    },
